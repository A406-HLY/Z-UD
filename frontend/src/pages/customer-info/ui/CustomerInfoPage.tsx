...
import { AuditReportSection } from '@/widgets/audit-result';
import { 
  useHouseAuditQuery, 
...
export const CustomerInfoPage = () => {
  ...
  // (Why) 현재 명세서 규격에 맞춘 샘플 데이터 요청
  const auditParams = {
    illegalBuilding: false,
    houseType: '아파트',
    propertyAddress: '서울특별시 강남구 테헤란로 212',
  };

  const { data: houseData, isLoading: isBackendLoading } = useHouseAuditQuery(auditParams);

  // (Why) 백엔드 SSE 이벤트를 모사하여 단계별 실시간 진행 상태 관리
  const { completedSteps, isAllDone } = useAuditProgress(isBackendLoading);
  ...
  const auditItems = useMemo<AuditSummaryItem[]>(() => {
    const effectiveHouseData = houseData || mockHouseData;
    const houseItems = effectiveHouseData ? mapHouseAuditToUiModel(effectiveHouseData) : [];
    
    const baseItems: AuditSummaryItem[] = [
      { id: 'credit-rating', title: '신용등급 조회', summary: 'A (최우수)', status: 'SUCCESS' },
      { id: 'loan-history', title: '기존 대출 내역', summary: '3건의 부채 확인', status: 'SUCCESS' },
      ...houseItems,
    ];

    return baseItems.map(item => ({
      ...item,
      status: completedSteps.has(item.id) ? item.status : 'LOADING',
    }));
  }, [houseData, mockHouseData, completedSteps]);

  // (Why) 모든 조회가 완료되었고, 'ERROR' 상태인 항목이 하나도 없을 때만 다음 단계로 진행 허용
  const canProceed = isAllDone && !auditItems.some(item => item.status === 'ERROR');
  ...
  const actionButton = useMemo(() => {
    if (!isAllDone) return { label: '심사 중...', onClick: () => {}, disabled: true, className: 'bg-slate-100 text-slate-400' };
