...
INPUT_PATH = {
    "worker_case": "./worker_input.json",
    "business_case": "./business_input.json",
}
...
def load_rule_index_from_txt(rule_txt_path: str) -> Dict[str, Dict[str, str]]:
    text = load_text(rule_txt_path).strip()
    matches = list(ARTICLE_HEADER_RE.finditer(text))
    if not matches:
        raise ValueError(f"조항 헤더를 찾지 못했습니다: {rule_txt_path}")
...
def run_single_input(input_path: str, metadata: dict, judge_criteria: str) -> dict:
    raw_input = load_json(input_path)
    output = {
        "consultationId": raw_input.get("consultationId"),
        "products": []
    }

    for product_code, product_block in raw_input.get("result", {}).items():
        if product_code not in metadata["products"]:
            continue
        rule_index = load_rule_index_from_txt(RULE_TXT_MAP[product_code])
        product_rule_text = load_text(RULE_TXT_MAP[product_code])
        output["products"].append({
            ...
            "forReport": run_for_report(product_code, product_block, metadata, product_rule_text, rule_index, judge_criteria),
        })
        cleanup_cuda()
...
def main() -> None:
    ...
    for case_name, input_path in INPUT_JSON_PATHS.items():
        result = run_single_input(input_path, metadata, judge_criteria)
        out_path = Path(OUTPUT_DIR) / f"{case_name}_output.json"
        with open(out_path, "w", encoding="utf-8") as f:
            json.dump(result, f, ensure_ascii=False, indent=2)
        print(f"[완료] {case_name} -> {out_path}")
