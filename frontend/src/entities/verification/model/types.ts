 ...
 * (Why: 백엔드 응답 snake_case 구조를 그대로 정의한 뒤 Mapper에서 처리하기 위함)
 */
...
export interface ServerDocItem {
  id: string;
  name: string;
  documentType: string; // documentTypeLabel 등 매핑용
  uploadTime: string;
  pageCount: number;
  size: string;
...
export interface DocItem extends Omit<ServerDocItem, 'fields'> {
