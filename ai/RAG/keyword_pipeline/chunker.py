...
class ArticleChunker:
    ...
    def run(self):
        ...
        chunks = []
        pattern = re.compile('^(제\\d+조\\s[^\\n]+)', re.MULTILINE)
        matches = list(pattern.finditer(content))
        for i, match in enumerate(matches):
            start = match.start()
            end = matches[i + 1].start() if i + 1 < len(matches) else len(content)
            header = match.group(0).strip()
            article_id_match = re.search('제\\d+조', header)
            article_id = article_id_match.group(0) if article_id_match else '제0조'
            article_title = header.strip()
            body = content[match.end():end].strip()
            chunks.append({'chunk_id': f'chunk_{i + 1:02d}', 'article_id': article_id, 'title': article_title, 'text': f'{article_title}\n{body}'})
        return chunks
