...
class KeywordExtractionPipeline:
    ...
    def run(self):
        ...
        for i, chunk in enumerate(chunks):
            ...
            validated = self.validator.validate(chunk_text, unique_candidates, threshold=SIMILARITY_THRESHOLD, top_k=len(unique_candidates))
            ...
            for word, count in unique_candidates:
                ...
                scored_keywords.append({'word': word, 'final_score': round(final_score, 2), 'rf': count, 'df': df, 'relevance': round(relevance, 4)})
            scored_keywords.sort(key=lambda x: x['final_score'], reverse=True)
            top_selection = scored_keywords[:TOP_K]
            import re
            clean_title = re.sub('^#+\\s*', '', chunk['title']).strip()
                ...
                if concept_key not in matched_concepts:
                    for kw in final_kws:
                        ...
                        if concept_key in matched_concepts:
                            break
            results.append({'chunk_id': chunk['chunk_id'], 'article_id': chunk['article_id'], 'title': chunk['title'], 'original_text': chunk_text, 'final_keywords': final_kws, 'concepts': list(matched_concepts)})
        return results

    def _empty_result(self, chunk):
        return {'chunk_id': chunk['chunk_id'], 'article_id': chunk['article_id'], 'title': chunk['title'], 'original_text': chunk['text'], 'final_keywords': [], 'concepts': []}
